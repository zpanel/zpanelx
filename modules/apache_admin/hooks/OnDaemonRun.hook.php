...
function WriteVhostConfigFile() {
    ...
    while ($rowvhost = $sql->fetch()) {
        ...
        if ($rowvhost['vh_enabled_in'] == 1 && ctrl_users::CheckUserEnabled($rowvhost['vh_acc_fk']) ||
            ...
            if ($vhostuser['diskquota'] != 0 and $diskspace > $vhostuser['diskquota']) {
            ...
            } elseif ($vhostuser['bandwidthquota'] != 0 and $bandwidth > $vhostuser['bandwidthquota']) {
                $line .= '# DOMAIN: ' . $rowvhost['vh_name_vc'] . fs_filehandler::NewLine();
    ...
    if (fs_filehandler::UpdateFile($vhconfigfile, 0777, $line)) {
        ...
        $time = time();
        $vsql = $zdbh->prepare('UPDATE x_settings SET so_value_tx=:time WHERE so_name_vc="apache_changed"');
