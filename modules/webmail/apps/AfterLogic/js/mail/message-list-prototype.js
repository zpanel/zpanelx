...
var MessageListPrototype = {
	...
	_fillMessageInfo: function(msg, hidePicturesControl) {
		...
		if (this._appointmentConfirmationControl) {
			if (msg.oAppointment !== null && msg.oAppointment.sType === EnumAppointmentType.Request) {
	...
	setVoiceMessageDuration: function (fSeconds)
	{
		this._voiceMessageViewPane.setDuration(fSeconds);
	},
	...
	ClearSearch: function() {
		var historyObj = this.getCurrFolderHistoryObject();
		var oFld = historyObj.oKeyMsgList.getFolder();
		if (oFld.id === -1) {
 ...
 */
	...
	FillByFolders: function() {
		...
		for (; iIndex < iLen; iIndex++) {
			...
			if (oAccount.bIgnoreSubscribeStatus || !oFolder.listHide) {
				...
				eMoveItem = {};
				if ((bNotDirectMode || bNotPop3) && oFolder.noselect === false) {
	...
	_fillByMessages: function() {
		...
		if (msgsArray.length == 0) {
			...
			var bHasViewMsg = this._inboxTable.fill(msgsArray, this.id, additionalMessage, addClearLink);
			if (!bHasViewMsg) {
				this.cleanMessageBody(true);
			}
			this.RedrawPages(this.oKeyMsgList.iPage);
