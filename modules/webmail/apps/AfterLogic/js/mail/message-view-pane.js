...
function CVoiceMessageViewPane(eContainer)
{
	this._bBuilded = false;
...
CVoiceMessageViewPane.prototype = {
	show: function (oMsg, iScreenId)
	{
		...
		if (WebMail.Settings.iFlashVersion < 10) {
	...
	showPlayer: function (oMsg, iScreenId)
	{
		...
		if (oMsg.voiceAttachment.sTranscription.length > 0) {
		...
		this._eDownloadLink.href = oMsg.voiceAttachment.sDownload;
		this.setDuration(oMsg.voiceAttachment.iDuration);

		var sVoiceLink = oMsg.voiceAttachment.sVoice;
		sVoiceLink = encodeURIComponent(sVoiceLink);
		var iWidth = 1;
		var iHeight = 1;
		var sPlayerLink = 'js/mail/wavplayer.swf?gui=full&amp;w=' + iWidth + '&amp;h=' + iHeight;

		var sFlashHtml = '<object';
	...
	setDuration: function (fSeconds)
	{
		this._fTotalSeconds = fSeconds;
		var iSeconds = Math.floor(fSeconds);
	...
	_markMessageAsRead: function ()
	{
		...
		oScreen.markMessageAsRead(this._sMsgId, this._iMsgSize, true);
		this._bMsgRead = true;
	},
...
function LoadVoiceHandler(fLoadedSeconds, fTotalSeconds)
{
	var oScreen = WebMail.getCurrentListScreen();
	if (oScreen == null) return;
	oScreen.setVoiceMessageDuration(fTotalSeconds);
}
