...
CEditContactScreenPart.prototype = {
	...
	saveChanges: function ()
	{
		...
		if (Validator.isEmpty(fullNameValue) && Validator.isEmpty(hEmailValue) &&
			...
			Dialog.alert(UseCustomContacts ? Lang.CantLeaveBlank : Lang.WarningContactNotComplete);
			return;
		}
		...
		if ((!Validator.isEmpty(hEmailValue) && !Validator.isCorrectEmail(hEmailValue)) ||
			(!Validator.isEmpty(bEmailValue) && !Validator.isCorrectEmail(bEmailValue)) ||
			(!Validator.isEmpty(oEmailValue) && !Validator.isCorrectEmail(oEmailValue)))
		{
			Dialog.alert(UseCustomContacts ? Lang.CantLeaveBlank : Lang.WarningCorrectEmail);
			return;
		}

		if (UseCustomContacts && (Validator.isEmpty(firstNameValue) || Validator.isEmpty(surNameValue))) {
			Dialog.alert(Lang.CantLeaveBlank);
			return;
		}
