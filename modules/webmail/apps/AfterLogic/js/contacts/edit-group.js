...
CEditGroupScreenPart.prototype = {
	...
	{
		/* validation */
		var sGroupId = this.Group.sGroupId;
		var name = (sGroupId.length === 0 || this._isEditName)
		...
		for (var i = 0; i < iCount; i++) {
			...
			if (cont.deleted == false) {
				group.contacts.push({ sContactId: cont.sContactId, name: cont.name, email: cont.email });
			}
		}
		...
		if (sGroupId.length === 0) {
			...
			for (i = 0; i < group.contacts.length; i++) {
				stringDataKey = WebMail.DataSource.getStringDataKey(TYPE_CONTACT, {sContactId: group.contacts[i].sContactId});
			...
			stringDataKey = WebMail.DataSource.getStringDataKey(group.type, {sGroupId: group.sGroupId});
			WebMail.DataSource.cache.replaceData(group.type, stringDataKey, group);
			WebMail.DataSource.cache.renameRemoveGroupInContacts(group.sGroupId, group.name, group.contacts);
