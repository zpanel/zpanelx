...
CNetLoader.prototype = {
	...
	hasOpenRequests: function ()
	{
		...
		for (var index in this._requests) {
			var requestData = this._requests[index];
			if (typeof(requestData) === 'function') continue;
			if (requestData.request.readyState != readyStateComplete) {
				return true;
			}
		}
		return false;
	...
	checkRequests: function ()
	{
		for (var index in this._requests) {
			...
			if (typeof(requestData) === 'function') continue;
			var date = new Date();
			var diff = date - requestData.date;
			if (diff > this._timeInterval) {
				this._abortRequest(index);
				this._onError.call({errorDesc: '', request: requestData.requestWord,
					action: requestData.actionWord});
			}
		}
	},

	loadXmlDoc: function (sPostParams, actionWord, requestWord)
	{
		var obj = this;
		if (!this._funcInterval) {
			this._timeInterval = 100000;
			this._funcInterval = setInterval(function () {obj.checkRequests();}, this._timeInterval);
		}
		...
		if (request) {
			try {
				...
				this._requests[index] = requestData;
				request.send(sPostParams);
			}
			catch (err) {
			...
			if (httpStatus == 200 || httpStatus == 0) {
				if (httpStatus != 0 || request.getResponseHeader('Content-Type') != null) {
					this._onLoad.call({responseXML: request.responseXML,
						responseText: request.responseText, request: requestData.requestWord,
						action: requestData.actionWord});
				}
			}
				...
				var errorDesc = Lang.ErrorConnectionFailed + ' (' + httpStatus + ' - ' +
					httpStatusText + ')';
				this._onError.call({errorDesc: errorDesc, request: requestData.requestWord,
					action: requestData.actionWord});
			}
	...
	{
		for (var index in this._requests) {
			if (typeof(this._requests[index]) === 'function') continue;
			this._abortRequest(index);
		}
	},

	_abortRequest: function (index)
	{
		var oRequestData = this._requests[index];
...
CLanguageChanger.prototype = {
	...
	_apply$: function (oItem)
	{
		switch (oItem.sType) {
			case 'html':
				oItem.$elem.html(Lang[oItem.sField]);
				break;
			case 'value':
				oItem.$elem.attr('value', Lang[oItem.sField]);
				break;
		}
	},
