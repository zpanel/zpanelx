...
CVariableTable.prototype = 
{
	...
	cleanLines: function (msg1, msg2)
	{
		this._clean();
		if (msg2 != undefined) {
			this.addMessageToList(msg1, false);
			msg1 = msg2;
		}
	...
	{
		this._clean();
		this._addClearSearchLink(this._eLinesCont);
		var div = CreateChild(this._eLinesCont, 'div');
		div.className = 'wm_inbox_info_message';
		div.innerHTML = Lang.InfoNoMessagesFound;
	},
	
	_addClearSearchLink: function (parent)
	{
		$('<a></a>')
			.attr({'href': '#', 'class': 'wm_search_result_clear_button'})
			.html(Lang.SearchClear)
			.click(function () {
	...
	fill: function (objsArr, screenId, msg, addClearLink, bNextPageMsgList)
	{
		if (!bNextPageMsgList) {
			this._scrollLines(0);
		...
		this.addMessageToList(msg, addClearLink);
...
//				span.style.paddingLeft = column._padding + 'px';
//				span.style.paddingRight = column._padding + 'px';
...
CSelection.prototype = 
{
	...
	{
		var iStartLineIdx = 0;
		for (var iMsgIdx = 0; iMsgIdx < oMsgsList.list.length; iMsgIdx++) {
			var oMsg = oMsgsList.list[iMsgIdx];
			var sId = oMsg.getIdForList(iListScreenId);
			for (var iLineIdx = iStartLineIdx; iLineIdx < this.iLength; iLineIdx++) {
				var oLine = this._lines[iLineIdx];
				if (sId === oLine.id) {
					if (oLine.Hidden) {
