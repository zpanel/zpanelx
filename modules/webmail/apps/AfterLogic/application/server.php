		...
		if ($oAccount)
		{
			...
			if (!empty($sTempFile) && !empty($sAction) && !empty($sCalendarId))
			{
				...
				if (!empty($sData))
				{
					...
					$bResult = $oCalendarApi->ProcessAppointment($oAccount, $sAction, $sCalendarId, $sData);
					if (true === $bResult)
					{
...
// OTHER
	...
	{
		$sToken = (string) CApi::CsrfBrowserToken($this->oInput);
		return !(!empty($sToken) && (string) $this->oRequestXml->GetParamValueByName('token') !== $sToken);
	}
