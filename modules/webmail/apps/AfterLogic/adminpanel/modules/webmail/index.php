...
class CWebMailModule extends ap_Module
{
	 ...
	 * @return bool
	 */
	public function LoginToAccount($iAccountId)
	{
		$bResult = false;
		if (!$this->oAdminPanel->IsSubAdminAuthType())
		{
			$oAccount = $this->oUsersApi->GetAccountById($iAccountId);
			if ($oAccount)
			{
				$oAccount->FillSession(true);
				$this->oWebmailApi->JumpToWebMail('../webmail.php?check=1');
				$bResult = true;
			}
		}
		return $bResult;
